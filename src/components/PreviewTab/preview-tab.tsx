...
const PreviewTab: React.FC<PreviewTabProps> = ({matchDetailsData, filteredData, matchIndex, bestOfMatch, redirectToTab, weatherData}) => {
    ...
    function createMatchInfos() {
        ...
        if (lastTwoGamesInFacility?.length > 1 && lastTwoGamesInFacility?.every((x: any) => x?.oyesfc?.goal >= x?.rival?.goal)) {
            const no4 = `O Yes FC did not lose the last 2 matches played in ${matchDetailsData?.place}.`
            infosForMatch.push(no4)
        }
        if (lastThreeGamesWithRival?.length > 2 && lastThreeGamesWithRival?.every((x: any) => x?.oyesfc?.goal >= x?.rival?.goal)) {
        ...
        const filteredNames = Object.entries(allTimePlayerGoals).filter((value: any) => {
            const remainder = value[1] < 50 ? 50 - value[1] : 50 - (value[1] % 50);
            return remainder >= 1 && remainder <= 3 && (value[1] + remainder) % 50 === 0;
        }).map(([name, value]: any) => ({name, remainder: value < 50 ? 50 - value : 50 - (value % 50), goal: value}));
        ...
        const filteredNamesOfMatches = Object.entries(allTimeMatchesCount).filter((value: any) => {
            return value[1] % 25 === 0;
                                        ...
                                        <Divider sx={{bgcolor: 'gray', margin: '10px'}}/>
                                        {
                                            Object.entries(facilityRatingData)?.map((a, b) => (
                                                <div key={b} className={matchDetailsClasses.generalInfoDiv}>
                                                    <span className={playerCardsClasses.ratingSpan} style={{
                                                        background: Number(Number(a[1])?.toFixed(0)) >= 7 ? 'darkgreen' :
                                                            Number(Number(a[1])?.toFixed(0)) < 5 ? 'darkred' : 'darkgoldenrod'
                                                    }}>
