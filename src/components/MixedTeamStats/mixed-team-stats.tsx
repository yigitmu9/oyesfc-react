...
const MixedTeamStats = () => {
    ...
    const tableColumnNames = useMemo(
        () => [selectedOption, 'Matches', 'Wins', 'Draws', 'Losses', 'Scored', 'Per Game', 'Conceded', 'Per Game'],
        [selectedOption]
    );
    ...
    const fetchData = useCallback(
        (option: any) => {
            ...
            if (option === options[0]) {
                ...
                setLabels(Jerseys);
                Jerseys.forEach((x) => {
                    const filteredDataConst = Object.values(filteredData)?.filter(
                        (item: any) => item.oyesfc.jersey === x
                    );
                    const calculatedData = calculateTeamStats(filteredDataConst);
                    const winRate = ((calculatedData.win / calculatedData.matches) * 100)?.toFixed(0);
                    const arrayData = Object.values(calculatedData).slice(0, -2);
                ...
                categoryValues.playerNumbers?.forEach((x) => {
                    const filteredDataConst = Object.values(filteredData)?.filter(
                        (item: any) => Object.keys(item?.oyesfc?.squad)?.length === x
                    );
                    const calculatedData = calculateTeamStats(filteredDataConst);
                    const winRate = ((calculatedData.win / calculatedData.matches) * 100)?.toFixed(0);
                    const arrayData = Object.values(calculatedData).slice(0, -2);
                ...
                categoryValues.weathers?.forEach((x) => {
                    const filteredDataConst = Object.values(filteredData)?.filter(
                        (item: any) => item?.weather?.weather === x || item?.weather?.sky === x
                    );
                    const calculatedData = calculateTeamStats(filteredDataConst);
                    const winRate = ((calculatedData.win / calculatedData.matches) * 100)?.toFixed(0);
                    const arrayData = Object.values(calculatedData).slice(0, -2);
        ...
        [
            categoryValues.facilities,
            categoryValues.playerNumbers,
            categoryValues.rivals,
            categoryValues.weathers,
            filteredData,
            options,
        ]
    );
