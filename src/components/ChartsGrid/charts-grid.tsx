...
const ChartsGrid = () => {
    ...
    useEffect(() => {
        if (filteredData) {
            ...
            const performance = calculatedRakipbulData?.map((x, y) =>
                (((calculatedNormalData[y][4] - x[4]) / x[4]) * 100)?.toFixed(0)
            );
            const finalData = {
                goalPerGameData: goalPerGameData,
    ...
    const firstCard = (
        <>
            ...
            <ChartComponent
                ...
                data={chartData?.goalPerGameData}
    ...
    const fourthCard = (
        <>
            <MainTitle
                title={'Regular Matches Compared to the Rakipbul Performance Increase Percentage'}
                size={'mid'}
            />
