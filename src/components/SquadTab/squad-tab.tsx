...
const SquadTab: React.FC<SquadTabProps> = ({matchDetailsData, squadRatings, bestOfMatch}) => {
    const { signedIn } = useSelector((state?: any) => state.credentials);
    let oyesfcSquad: any;
    let playerColor: any;
    let playerNumberColor: any;
    let gkColor: any;
    let gkNumberColor: any;
    ...
    if (Object.values(matchDetailsData.oyesfc.squad).every(x => x?.hasOwnProperty('role'))) {
        Object.entries(matchDetailsData.oyesfc.squad)?.forEach((x: any) => {
            let player: any;
        ...
        <div className={squadTabClasses.fieldBox}>
            <Box className={squadTabClasses.footballField}>
                                    ...
                                    <div
                                        style={{
                                            background: bestOfMatch?.name === oyesfcSquad?.squad?.gk?.fullName ? 'royalblue' :
                                                squadRatings?.find((rating: any) => rating?.name === oyesfcSquad?.squad?.gk?.fullName)?.rating >= 7 ? 'darkgreen' :
                                                    squadRatings?.find((rating: any) => rating?.name === oyesfcSquad?.squad?.gk?.fullName)?.rating < 5 ? 'darkred' : 'darkgoldenrod'
                                        }}
                                        ...
                                        <span className={squadTabClasses.spanStyle}>
                                            {squadRatings?.find((rating: any) => rating?.name === oyesfcSquad?.squad?.gk?.fullName)?.rating.toFixed(1)}
                                        </span>
                ...
                <Box className={squadTabClasses.defense}>
                    {oyesfcSquad?.squad?.df?.sort((a: any, b: any) => b?.position - a?.position)?.map((x: any, y: number) => (
                                    ...
                                    <div
                                        style={{
                                            background: bestOfMatch?.name === x?.fullName ? 'royalblue' :
                                                squadRatings?.find((rating: any) => rating?.name === x?.fullName)?.rating >= 7 ? 'darkgreen' :
                                                    squadRatings?.find((rating: any) => rating?.name === x?.fullName)?.rating < 5 ? 'darkred' : 'darkgoldenrod'
                                        }}
                                        className={bestOfMatch?.name === x?.fullName ? squadTabClasses.momRating : squadTabClasses.rating}>
                                        <span className={squadTabClasses.spanStyle}>
                                            {squadRatings?.find((rating: any) => rating?.name === x?.fullName)?.rating.toFixed(1)}
                                        </span>
                ...
                {oyesfcSquad?.squad?.cm?.sort((a: any, b: any) => b?.position - a?.position)?.length > 0 &&
                    <Box className={squadTabClasses.midfield}>
                        {oyesfcSquad?.squad?.cm?.map((x: any, y: number) => (
                            <div key={y} className={squadTabClasses.playerDesign}>
                            <Badge
                                    overlap="circular"
                                    anchorOrigin={{vertical: 'top', horizontal: 'right'}}
                ...
                <Box className={squadTabClasses.forward}>
                    {oyesfcSquad?.squad?.fw?.sort((a: any, b: any) => b?.position - a?.position)?.map((x: any, y: number) => (
                        <div key={y} className={squadTabClasses.playerDesign}>
                            <Badge
                                overlap="circular"
                                anchorOrigin={{vertical: 'top', horizontal: 'right'}}
