...
const RivalComparison = ({data, selectedRival}) => {
    ...
    const rivalData = Object.values(data).filter(x => x.rival.name === selectedRival)
    const calculatedData = calculateTeamStats(rivalData)
    const winRateStat = calculatedData.win !== 0 ? ((calculatedData.win / calculatedData.matches) * 100)?.toFixed(0) : '0';
    const loseRateStat = calculatedData.lose !== 0 ? ((calculatedData.lose / calculatedData.matches) * 100)?.toFixed(0) : '0';


    const readyData = [
        [winRateStat, 'Win Rate (%)', loseRateStat,
            calculateRateInfo(winRateStat, loseRateStat),
            100 - Number(calculateRateInfo(winRateStat, loseRateStat))],

        [calculatedData.matches, 'Matches', calculatedData.matches,
        ...
        [calculatedData.win, 'Wins', calculatedData.lose,
        ...
        [calculatedData.draw, 'Draws', calculatedData.draw,
        ...
        [calculatedData.lose, 'Losses', calculatedData.win,
        ...
        [calculatedData.scoredGoal, 'Goals Scored', calculatedData.concededGoal,
        ...
        [calculatedData.concededGoal, 'Goals Conceded', calculatedData.scoredGoal,
        ...
        [calculatedData.goalDifference, 'Goal Difference', -Number(calculatedData.goalDifference),
            calculatedData.goalDifference > 0 ? 100 : 0,
            100 - (calculatedData.goalDifference > 0 ? 100 : 0)],

        [calculatedData.scoredGoalsPerGame, 'Scored per Match', calculatedData.concededGoalsPerGame,
        ...
        [calculatedData.concededGoalsPerGame, 'Conceded per Match', calculatedData.scoredGoalsPerGame,
        ...
        [calculatedData.goalDifferencePerGame, 'Goal Difference per Match', -Number(calculatedData.goalDifferencePerGame),
            calculatedData.goalDifferencePerGame > 0 ? 100 : 0,
            100 - (calculatedData.goalDifferencePerGame > 0 ? 100 : 0)],
