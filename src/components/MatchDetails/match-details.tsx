...
export const MatchDetails: React.FC<MatchDetailsProps> = ({matchDate, cameFrom}) => {
    ...
    const calculatePlayerRatings = (ratingResponse?: any) => {
        ...
        Object.entries(matchDetailsData.oyesfc.squad).forEach((playerName: any) => {
            ...
            const averageRating = points / Object.entries(ratingResponse)?.filter((z: any) => Object.keys(z[1]).includes(playerName[0])).length;
    ...
    const submitNote = async () => {
        if (noteFormData['note']?.length > 0) {
            ...
            try {
                ...
                setMatchNotes((prevData: any) => ([
                    ...prevData?.filter((x: any) => x[0] !== userName),
                    newNote
                ]));
    ...
    const getOpenWeather = async () => {
        if (matchDetailsData?.day && matchDetailsData?.place) {
            ...
            if (dayDifference <= 5) {
                ...
                const specificForecast = type === openWeatherType.forecast ? weatherResponse1?.list?.find((forecast: any) =>
                    forecast.dt_txt === weatherDate) : weatherResponse1;
