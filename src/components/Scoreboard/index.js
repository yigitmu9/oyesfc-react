...
const Scoreboard = ({value, openPopup}) => {

    const [buttonBgColor, setButtonBgColor] = useState('#242424');

    const handleButtonHover = () => {
        setButtonBgColor('#323232'); // New background color when hovering
    };

    const handleButtonLeave = () => {
        setButtonBgColor('#242424'); // Reset back to default color when not hovering
    };

    return (
        <section className={classes.scoreboard} onClick={openPopup} onMouseEnter={handleButtonHover}
