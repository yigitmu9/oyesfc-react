...
const SortingGrid: React.FC<SortingGridProps> = ({category, handlePreviousPage}) => {
    ...
    const returnPlayerStatColors = (data: any) => {
        if (selectedOption === 'Goals per Game' ||
        ...
        if (selectedOption === 'Average Match Rating') {
            return Number(Number(data[selectedOption])?.toFixed(0)) >= 7 ? 'darkgreen' :
                Number(Number(data[selectedOption])?.toFixed(0)) < 6 ? 'darkred' : 'darkgoldenrod'
        }
        return Number(Number(data[selectedOption])?.toFixed(0)) >= 80 ? 'darkgreen' :
            Number(Number(data[selectedOption])?.toFixed(0)) < 60 ? 'darkred' : 'darkgoldenrod'
    }
