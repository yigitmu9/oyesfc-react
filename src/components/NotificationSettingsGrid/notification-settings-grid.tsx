...
const NotificationSettingsGrid: React.FC<NotificationSettingsGridProps> = ({handlePreviousPage}) => {
    ...
    const handleSubmit = async () => {
        setFormLoading(true)
        const playerIds = Object.entries(notificationsData)?.filter((a: any) => formData?.[a[0]] === true)
                ...
                <>
                    ...
                    <form className={classes.formStyle}>
                        <div>
                            ...
                            <div style={{height: '20px'}}></div>
                            {
                                isCaptain && notificationsData &&
                                Object.keys(notificationsData)?.filter(a => OYesFCPlayersArray.includes(a))?.map((x, y) => (
                                    <div key={y}>
                            ...
                            {errorMessage?.severity &&
                                <Alert sx={{
                                    ...
                                    color: 'lightgray'
                                       ...
                                       variant="outlined"
                                       severity={errorMessage?.severity}>{errorMessage?.message}</Alert>}
                        </div>
